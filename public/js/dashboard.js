...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterTasks() {
      ...
      taskItems.forEach(item => {
        const taskType = item.getAttribute('data-type');
        const taskDate = item.getAttribute('data-date');
      ...
      // Update UI to show empty state if no tasks match
      const visibleTasks = document.querySelectorAll('.task-item:not(.d-none)');
      const emptyMessage = document.getElementById('emptyTasksMessage');
      
      if (visibleTasks.length === 0 && emptyMessage) {
        emptyMessage.classList.remove('d-none');
        emptyMessage.textContent = `No ${filterValue === 'all' ? '' : filterValue} tasks found.`;
      } else if (emptyMessage) {
        emptyMessage.classList.add('d-none');
      }
    }
    
    // Function to setup task checkboxes
        ...
        checkbox.addEventListener('change', function() {
          ...
          fetch(`/tasks/${taskId}/status`, {
            ...
            .catch(error => {
              ...
              this.checked = !completed; // Revert checkbox state
            });
        });
      });
    }
    
    // Function to setup task edit buttons
